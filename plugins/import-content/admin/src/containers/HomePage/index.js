...
class HomePage extends React.Component {  
  ...
  componentDidMount() {
    this.getModels().then(res => { console.log(res);
  ...
  getModels = async () => {  
    ...
    try {
      const response = await request(api_url + 'content-type-builder/content-types', {
        method: "GET"
      });
  ...
  onRequestAnalysis = async analysisConfig => {  
    ...
    this.setState({ analyzing: true }, async () => {
      ...
      try {
        const response = await request(api_url + "import-content/preAnalyzeImportFile", {
          method: "POST",
          body: analysisConfig
        });
